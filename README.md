...
每个专业助理负责特定领域的操作，敏感操作需要人工确认（人机协作）。

### 状态管理

使用 LangGraph 的检查点机制进行状态持久化：
...
```bash
docker build -t ctrip-assistant .
docker run -p 8000:8000 ctrip-assistant
```
...
1. ⚠️ **修改默认 JWT 密钥** - 生产环境必须使用强密钥
2. 🔐 **使用强密码** - 数据库用户使用强密码
3. 🌐 **启用 HTTPS** - 生产环境必须使用 HTTPS
4. 🔑 **保护 API 密钥** - 不要将密钥提交到版本控制
5. 📝 **审查 CORS 设置** - 限制允许的源
